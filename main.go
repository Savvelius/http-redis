...
func main() {
	...
	r.Post("/quit/{username}:{password}", global.ClearUserData)
	...
	// invariant: when request is handled by any of those handlers:
	// user data is in Global and user is active
	// when user has an environment, user is active
	// authorizationMiddleware is kinda reverse of clearUserData
	...
	fmt.Printf("Listening and serving http at port %s\n", PORT)
